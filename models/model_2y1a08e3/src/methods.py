...
def train_model(model, criterion, optimizer, train_dataloader):
    ...
    outputs = outputs.reshape(B * W * H, C)
    B, C, W, H = label.shape
...
def training_loop(model, criterion, optimizer, train_dataloader, val_dataloader, dbx_access_token, num_epochs=50, critiqueing_metric="Mean Pixel Accuracy", auto_stop=True, auto_stop_patience=15):
