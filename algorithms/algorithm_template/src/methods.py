...
def test_algorithm_on_benchmarks(all_benchmarks=True, benchmarks=None, report_results=True,
    ...
    for benchmark in benchmarks:
        ...
        # benchmark_dataloader = DataLoader(benchmark_dataset, batch_size=1, shuffle=False)

        TN_total, FP_total, FN_total, TP_total = 0, 0, 0, 0
        # for data, label in benchmark_dataloader:
        for data, label in benchmark_dataset:
            output = lane_detector(data)
            label_binary = label.argmax(dim=1).cpu()
            conf_matrix = confusion_matrix(label_binary, output)
